...
public class AppHemo extends JFrame {
    ...
    private String rootPath = "/home/fernando/projeto_hemodinamica/testes/test_data";
    ...
    private void setupUI() {
        ...
        modelPendentes = new DefaultTableModel(new String[]{"Nome", "Arquivo", "Origem", "Procedimento", "Status"}, 0) {
            @Override public boolean isCellEditable(int r, int c) { return false; }
        };
        tablePendentes = new JTable(modelPendentes);
        tablePendentes.setRowHeight(40);
        tablePendentes.setFont(new Font("Arial", Font.PLAIN, 14));
        tablePendentes.getColumnModel().getColumn(0).setPreferredWidth(300);
        tablePendentes.getColumnModel().getColumn(1).setMinWidth(0);
        tablePendentes.getColumnModel().getColumn(1).setMaxWidth(0);
        tablePendentes.getColumnModel().getColumn(2).setPreferredWidth(120);
        tablePendentes.getColumnModel().getColumn(3).setPreferredWidth(200);
        tablePendentes.getColumnModel().getColumn(4).setPreferredWidth(150);

        // --- Renderizador Visual de Status ---
        tablePendentes.setDefaultRenderer(Object.class, new javax.swing.table.DefaultTableCellRenderer() {
            @Override
            public Component getTableCellRendererComponent(JTable table, Object value, boolean isSel, boolean hasFocus, int row, int col) {
                ...
                if (isSel) {
                    ...
                    if (status.equals("AGUARDANDO LAUDO")) {
                        c.setBackground(new Color(200, 255, 200)); // Verde claro
                        c.setForeground(new Color(0, 100, 0));
                    } else if (status.equals("EM SALA")) {
                        c.setBackground(new Color(255, 245, 200)); // Amarelo suave
                        c.setForeground(new Color(150, 100, 0));
                    } else if (status.equals("LAUDO DADO")) {
                        c.setBackground(new Color(200, 230, 255)); // Azul claro
                        c.setForeground(new Color(0, 50, 150));
                    } else {
        ...
        JButton btnLaudoCat = new JButton("LAUDO CAT");
        btnLaudoCat.setFont(new Font("Arial", Font.BOLD, 14));
        btnLaudoCat.setBackground(new Color(0, 102, 204));
        btnLaudoCat.setForeground(Color.WHITE);
        btnLaudoCat.setOpaque(true);
        btnLaudoCat.addActionListener(e -> new Thread(() -> fluxoLaudoIndiv("CATETERISMO")).start());

        JButton btnLaudoAngio = new JButton("LAUDO ANGIO");
        btnLaudoAngio.setFont(new Font("Arial", Font.BOLD, 14));
        btnLaudoAngio.setBackground(new Color(204, 0, 0));
        btnLaudoAngio.setForeground(Color.WHITE);
        btnLaudoAngio.setOpaque(true);
        btnLaudoAngio.addActionListener(e -> new Thread(() -> fluxoLaudoIndiv("ANGIOPLASTIA")).start());
        ...
        panelBotoes.setPreferredSize(new Dimension(0, 60));
        ...
        gbc.gridx = 0;
        gbc.weightx = 0.4;
        panelBotoes.add(btnLaudoCat, gbc);
        gbc.gridx = 1;
        gbc.weightx = 0.4;
        panelBotoes.add(btnLaudoAngio, gbc);
        gbc.gridx = 2;
        gbc.weightx = 0.2;
    ...
    private void fluxoLaudoIndiv(String tipoDesejado) {
        ...
        String origem = (String) modelPendentes.getValueAt(row, 2);
        String statusPac = (String) modelPendentes.getValueAt(row, 4);

        if (statusPac.equals("EM SALA")) {
            JOptionPane.showMessageDialog(this, "Paciente ainda está em mesa! Aguarde finalizar no tablet.", "Aviso", JOptionPane.WARNING_MESSAGE);
            return;
        }

        try {
            Map<String, String> dadosExtras = new HashMap<>();
            boolean evoluiuAngio = false;
            
            setStatus("Buscando dados...");
            String jsonStr = httpGet(serverUrl + "/api/baixar/json/" + encodePath(nomeArquivo));
            JSONObject jsonTablet = new JSONObject(jsonStr);
            evoluiuAngio = jsonTablet.optBoolean("evoluiu_angioplastia", false);
            
            if (tipoDesejado.equals("ANGIOPLASTIA") && !evoluiuAngio) {
                JOptionPane.showMessageDialog(this, "Não houve evolução para Angioplastia neste procedimento.", "Aviso", JOptionPane.INFORMATION_MESSAGE);
                return;
            }

            JSONArray matCatArr = jsonTablet.optJSONArray("materiais_cateterismo");
            String matCat = matCatArr != null ? String.join("\n", matCatArr.toList().stream().map(Object::toString).toArray(String[]::new)) : "";
            
            JSONArray matAngioArr = jsonTablet.optJSONArray("materiais_angioplastia");
            String matAngio = matAngioArr != null ? String.join("\n", matAngioArr.toList().stream().map(Object::toString).toArray(String[]::new)) : "";

            dadosExtras.put("{{MATERIAIS_CAT}}", matCat);
            dadosExtras.put("{{MATERIAIS_ANGIO}}", matAngio);
            dadosExtras.put("{{PROCEDENCIA}}", jsonTablet.optString("procedencia", ""));
            dadosExtras.put("{{CNS}}", jsonTablet.optString("cns", ""));
            dadosExtras.put("{{NASC}}", jsonTablet.optString("nasc", ""));

            String numExame = calcularProximoNumero();
            String label = tipoDesejado.equals("CATETERISMO") ? "CAT" : "ANGIO";
            String numConfirmado = JOptionPane.showInputDialog(this, "Número do exame (" + label + "):", numExame);
            if (numConfirmado == null) return;

            if (tipoDesejado.equals("CATETERISMO")) {
                setStatus("Gerando Cateterismo...");
                Map<String, String> dadosCat = new HashMap<>(dadosExtras);
                dadosCat.put("{{MATERIAIS}}", dadosExtras.getOrDefault("{{MATERIAIS_CAT}}", ""));
                File laudoCat = gerarDocumentoComMateriais("cateterismo", numConfirmado, nomeArquivo, nomePacienteLista, false, dadosCat);
                abrirArquivo(laudoCat);
            } else {
                setStatus("Gerando Angioplastia...");
                Map<String, String> dadosAngio = new HashMap<>(dadosExtras);
                // No laudo de angio, costuma-se colocar todos os materiais
                String todosMateriais = (dadosExtras.getOrDefault("{{MATERIAIS_CAT}}", "") + "\n" + dadosExtras.getOrDefault("{{MATERIAIS_ANGIO}}", "")).trim();
                dadosAngio.put("{{MATERIAIS}}", todosMateriais);
                File laudoAngio = gerarDocumentoComMateriais("angioplastia", numConfirmado, nomeArquivo, nomePacienteLista, true, dadosAngio);
                abrirArquivo(laudoAngio);
            }

            // Notifica conclusão NO SERVIDOR mas NÃO remove da tabela local
            // para permitir que o usuário abra o segundo laudo (se houver).
            new Thread(() -> notifyLaudoDone(nomeArquivo)).start();
            
            SwingUtilities.invokeLater(() -> {
                modelPendentes.setValueAt("LAUDO DADO", row, 4);
                refreshListHistorico();
                setStatus("Laudo " + label + " aberto.");
            });
    ...
    private File gerarDocumentoComMateriais(String tipo, String numeroExame, String nomeArquivoFicha, String nomePacienteFallback, boolean sufixoPtca, Map<String, String> extras) throws Exception {
        ...
        Map<String, String> dados = new HashMap<>(extras);
        dados.put("{{NUM_EXAME}}", numeroExame);
        dados.put("{{DATA_HOJE}}", LocalDate.now().format(DateTimeFormatter.ofPattern("dd/MM/yyyy")));
        dados.put("{{NOME}}", toPascalCase(nomePacienteFallback));
        // Garante preenchimento de CNS e NASC se disponíveis
        if (extras.containsKey("{{CNS}}")) dados.put("{{CNS}}", extras.get("{{CNS}}"));
        if (extras.containsKey("{{NASC}}")) {
             dados.put("{{NASC}}", extras.get("{{NASC}}"));
             dados.put("{{NASCIMENTO}}", extras.get("{{NASC}}"));
        }
        
        File pastaFinal = criarEstruturaDePastas(numeroExame, nomePacienteFallback, sufixoPtca);
        try (FileInputStream fis = new FileInputStream(modeloTemp);
             XWPFDocument doc = new XWPFDocument(fis)) {
            
            substituirPlaceholders(doc, dados);
            
            String nomeSaida = nomePacienteFallback.replaceAll("[^a-zA-Z0-9 .\\-]", "") + (sufixoPtca ? " PTCA" : "") + ".docx";
            File arquivoFinal = new File(pastaFinal, nomeSaida);
    ...
    private void notifyLaudoDone(String filename) {
        new Thread(() -> {
            try {
                URL url = new URL(serverUrl + "/api/finalizar_laudo/" + encodePath(filename));
    ...
    private void refreshListPendentes() {
        new Thread(() -> {
            try {
                String json = httpGet(serverUrl + "/api/pendentes?client=java");
    ...
    private void substituirNoParagrafo(XWPFParagraph p, Map<String, String> dados) {
        String textoCompleto = p.getText();
        if (textoCompleto == null || textoCompleto.isEmpty()) return;

        boolean mudou = false;
        for (Map.Entry<String, String> entry : dados.entrySet()) {
            if (textoCompleto.contains(entry.getKey())) {
                textoCompleto = textoCompleto.replace(entry.getKey(), entry.getValue() != null ? entry.getValue() : "");
                mudou = true;
            }
        }
        ...
        if (mudou) {
            ...
            // Adiciona o novo texto tratando quebras de linha
